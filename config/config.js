...
const DEFAULTS = {
    ...
    elevenlabs_api_key: "",
    openai_api_key: "",
    // google is the default because it's the one free engine verified to work from a
    // plain static page (Edge's trial WebSocket frequently 403s on Origin/handshake
    // checks). Hindi voice preselected to match the default language (hi-IN).
    tts_engine: "google", // edge | google | elevenlabs | openai
    voice_id: "hi",
    selected_voice: "Google Hindi", // name of the selected voice, for display
    language: "hi-IN",
    ...
    // true so a fresh install has a working voice out of the box
    // (Google Hindi, free and keyless). The user can change it anytime.
...
export class AirCConfig {
    constructor() {
        ...
        // Voice migration (v2): older configs are stuck on "edge" (the old default),
        // which often 403s from a static page. Only touch configs that never got a
        // voice explicitly chosen — if the user picked a voice themselves, keep it.
        if (this._data.tts_engine === "edge" && !this._data.voice_setup_complete) {
            this._data.tts_engine = "google";
            this._data.voice_id = "hi";
            this._data.selected_voice = "Google Hindi";
            this._data.voice_setup_complete = true;
            this.save();
        }
    }
