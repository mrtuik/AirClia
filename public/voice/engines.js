...
export const ENGINE_INFO = {
    airclia: { label: "AirClia Cloud", needsKey: false, free: true },
    ...
    browser: { label: "Device voice", needsKey: false, free: true }
};

export const ENGINE_ORDER = ["airclia", "edge", "google", "elevenlabs", "openai", "browser"];
...
export const BUILT_IN_VOICES = {
    airclia: [
        { id: "alloy", name: "Alloy", description: "Neutral and balanced. Secure built-in voice, no API key needed.", lang: "" },
        { id: "nova", name: "Nova", description: "Warm and energetic. Secure built-in voice.", lang: "" },
        { id: "shimmer", name: "Shimmer", description: "Soft and expressive. Secure built-in voice.", lang: "" },
        { id: "echo", name: "Echo", description: "Confident, mid-register. Secure built-in voice.", lang: "" },
        { id: "fable", name: "Fable", description: "Bright storyteller tone. Secure built-in voice.", lang: "" },
        { id: "onyx", name: "Onyx", description: "Deep and steady. Secure built-in voice.", lang: "" }
    ],
    elevenlabs: [
    ...
    browser: [
        { id: "device", name: "Device voice", description: "Uses the voice installed in this browser or phone.", lang: "" }
    ]
};
...
// Network-free fallback. It uses the browser/phone's own speech engine, so it
// still works when a provider is blocked by CORS, quota, or regional limits.
class SpeechSynthesisAudio extends EventTarget {
    constructor(text, lang = "en-US") {
        super();
        this.text = text;
        this.lang = /^[a-z]{2,3}(-[A-Z]{2})?$/.test(lang || "") ? lang : "en-US";
        this.currentTime = 0;
    }
    ...
    play() {
        if (!window.speechSynthesis || !window.SpeechSynthesisUtterance) {
            this.dispatchEvent(new Event("error"));
            return Promise.reject(new Error("This browser does not support device speech."));
        }
        ...
        utterance.rate = 1;
        utterance.pitch = 1;
        utterance.onstart = () => this.dispatchEvent(new Event("play"));
        utterance.onend = () => this.dispatchEvent(new Event("ended"));
        utterance.onerror = () => this.dispatchEvent(new Event("error"));
...
async function synthBrowser(text, lang) {
    return new SpeechSynthesisAudio(text, lang);
}
...
// ---------------------------------------------------------------------
// AirClia Cloud — synthesized by this app's own server endpoint, so no
// API key ever reaches the browser. Keyless from the client's point of view.
// ---------------------------------------------------------------------
async function synthAirclia(text, voiceId) {
    const response = await fetch("/api/public/tts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, voice: voiceId || "alloy" })
    });
    if (!response.ok) {
        let detail = "";
        try { detail = (await response.json()).error || ""; } catch (e) { detail = await response.text().catch(() => ""); }
        throw new Error(detail || `Voice service error ${response.status}`);
    }
    const blob = await response.blob();
    if (!blob || blob.size < 256) throw new Error("Voice service returned empty audio.");
    return new Audio(URL.createObjectURL(blob));
}
...
export async function synthesize(engine, text, voiceId, apiKey) {
    switch (engine) {
        case "airclia": return synthAirclia(text, voiceId);
        ...
        case "browser": return synthBrowser(text, voiceId);
