...
export class AirCVoice {
    ...
    async speak(text) {
        ...
        try {
            const audio = await this._synthesizeWithRetry(engine, text, voiceId, myGen);
    ...
    _bindAndPlay(audio, myGen) {
        ...
        const p = audio.play();
        if (p && typeof p.catch === "function") p.catch(() => { /* handled via 'error' listener */ });
    }
    ...
    async testVoice(sampleText = "Hey, this is what I sound like.", override = null) {
        ...
        const audio = await synthesize(engine, sampleText, voiceId, apiKey);
        this._bindAndPlay(audio, myGen);
    }
}
